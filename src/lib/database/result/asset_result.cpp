...
chain::asset deserialize_tx(const Iterator first)
{
    chain::asset tx;
...
asset_result::operator bool() const
{
    return slab_ != nullptr;
}

size_t asset_result::height() const
{
...
size_t asset_result::index() const
{
    BITCOIN_ASSERT(slab_);
    const auto memory = REMAP_ADDRESS(slab_);
...
chain::asset asset_result::asset() const
{
    BITCOIN_ASSERT(slab_);
    const auto memory = REMAP_ADDRESS(slab_);
